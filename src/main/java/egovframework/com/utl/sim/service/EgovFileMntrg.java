...
public class EgovFileMntrg extends Thread {
    ...
    protected EgovFileMntrg(String filename, File logFile) {
        ...
        for (int i = 0; i < fList.length; i++) {
            realOriginalList.add(fList[i].getAbsolutePath() + "$" + getLastModifiedTime(fList[i]) + "$" + ((fList[i].length() / 1024) > 0
                    ? (fList[i].length() / 1024) : 1) + "KB");
            writeLog("ORI_" + fList[i].getAbsolutePath() + "$" + getLastModifiedTime(fList[i]) + "$" + ((fList[i].length() / 1024) > 0
                    ? (fList[i].length() / 1024) : 1) + "KB");
        }
    ...
    protected void checkAndConfigure() {
        ...
        try {
            ...
            for (int i = 0; i < fList.length; i++) {
                currentList.add(fList[i].getAbsolutePath() + "$" + getLastModifiedTime(fList[i]) + "$" + ((fList[i].length() / 1024) > 0
                        ? (fList[i].length() / 1024) : 1) + "KB");
            }
            /*
    ...
    public boolean isEnd() {
        ...
        try {
            ...
            try {
                throw new java.io.FileNotFoundException("File IO exception");
