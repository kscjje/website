...
public class EgovPopupManageServiceImpl extends EgovAbstractServiceImpl implements EgovPopupManageService {
    ...
    public void deletePopup(PopupManageVO popupManageVO) throws Exception {
        deleteFile(popupManageVO);
        dao.deletePopup(popupManageVO);
    }

    /**
     * 기 등록된 팝업정보의 이미지파일을 삭제한다.
     * 
     * @param popupManageVO
     *            - 팝업 model
     */
    public void deleteFile(PopupManageVO popupManageVO) throws Exception {
        String realPath = FileMngUtil.GetRealUploadPath();
        FileVO fileVO = (FileVO) dao.selectFile(popupManageVO);
        if (fileVO != null) {
            File file = new File(realPath + fileVO.getFilePath(), fileVO.getFileName());

            if (file.delete()) {
                log.debug("[file.delete] file : File Deletion Success");
            } else {
                log.error("[file.delete] file : File Deletion Fail");
            }
        }
    }
     ...
     */
    @Override
    public void updateStatPopup(PopupManageVO popupManageVO) throws Exception {
        dao.updateStatPopup(popupManageVO);
    }
