...
public class EgovXssChecker {
    ...
    public static boolean checkerUserXss(HttpServletRequest request, String sUniqId) throws Exception {
        ...
        try {
            ...
            if (bLog) {
                log.debug("@Step1. XSS Check uniqId  : {}", sUniqId);
                log.debug("Step2. XSS Session uniqId  : {}", loginVO.getId());
                log.debug("Step3. XSS Session getUniqId  : {}", loginVO.getUniqId());
                log.debug("Step4. XSS Session getAuthorities  : {}", EgovUserDetailsHelper.getAuthorities());
            }

            // 체크 값에 대한 무결성 체크
            if (sUniqId == null || (loginVO == null
                    ? "" : EgovStringUtil.isNullToString(loginVO.getUniqId())) == null) {
