 ...
 *   2021. 7. 14.   전영석        SMS UMS Resource 변경
 *      </pre>
 */
public abstract class EgovComSmsUmsAbstractDAO extends EgovAbstractMapper {

    //// private final Logger LOGGER = LoggerFactory.getLogger(this.getClass());

    @Resource(name = "dataSourceSMSSqlSession")
     ...
     *            - 단건 조회 처리 SQL mapping 쿼리 ID
     * @param parameterObject
     *            - 맵 조회 처리 SQL mapping 입력 데이터(조회 조건)를 세팅한 파라메터 객체(보통 VO 또는 Map)
     * @param mapKey
     *            - 결과 객체의 프로퍼티 중 하나를 키로 사용
     * @return 결과 객체 - SQL mapping 파일에서 지정한 resultType/resultMap 에 의한 단일 결과 객체(보통 VO 또는 Map)의 Map
     */
     ...
     *            - 리스트 조회 처리 SQL mapping 쿼리 ID
     * @param handler
     *            - 조회 결과를 제어하기 위해 구현한 ResultHandler
     * @return
     * @return 결과 List 객체 - SQL mapping 파일에서 지정한 resultType/resultMap 에 의한 결과 객체(보통 VO 또는 Map)의 List
     */
    @Override
