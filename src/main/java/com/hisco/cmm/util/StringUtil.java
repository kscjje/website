...
public class StringUtil {
    ...
    public static int String2Int(String str, int def) {
        ...
        String strThisValue = str;

        if (IsEmpty(strThisValue)) {
            return def;
        }

        try {
            try {
                return Integer.parseInt(strThisValue, 10);
            } catch (Exception e) {
                if (strThisValue.indexOf(".") > -1) {
                    strThisValue = strThisValue.substring(0, strThisValue.indexOf("."));
                }
                return Integer.valueOf(strThisValue.replaceAll("\\D", ""));
            }
        } catch (Exception e2) {
            return def;
        }
    ...
    public static long String2Long(String str, long def) {
        ...
        String strThisValie = str;

        if (IsEmpty(strThisValie)) {
            return def;
        }

        try {
            try {
                return Long.parseLong(strThisValie, 10);
            } catch (Exception e) {
                if (strThisValie.indexOf(".") > -1) {
                    strThisValie = strThisValie.substring(0, strThisValie.indexOf("."));
                }
                return Long.valueOf(strThisValie.replaceAll("[^\\-0-9]", ""));
            }
        } catch (Exception e) {
            return def;
        }
    ...
    public static boolean ContainsValueNocase(String[] arr, String val) {
        ...
        if (arr != null && arr.length > 0 && !IsEmpty(strThisVal)) {
            ...
            for (int i = 0, length = arr.length; i < length; i++) {
                if (!IsEmpty(arr[i]) && EqualsNotCase(arr[i], strThisVal)) {
                    ret = true;
                    break;
                }
            }
     ...
     * {
     * if (StringUtil.IsEmpty(val)) return val;
     * //return StringEscapeUtils.esescapeXml(val); JYS 2021.06.06
    ...
    public static String subStrBytes(String source, int cutLength) {
        ...
        // System.out.println("source.getBytes().length = " + source.getBytes().length);
    ...
    public static String padLeftZeros(int seq, int length) {
        String inputString = String.valueOf(seq);
    ...
    /**
	 * String이 비었거나("") 혹은 null 일경우 true 반환 Trim 처리 안함
	 */
