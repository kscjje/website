...
<script type='text/javascript'>
	...
	var fileGuide  = "<c:out value="${param.file_guide}"/>";
	var fileMaxSize = 10;
...
<c:forEach items="${fileList}" var="item" varStatus="status">
			<li>
				<span><c:out value="${item.orginFileName}"/><a href="javascript:;" onclick="fn_file_delete(this,'${item.fileSn}')" style="margin-left:5px" class="badge bg-danger">X</a></span>
				<c:if test="${param.show_image eq 'Y' }">
					...
					<br>
					<img src="<c:url value="/web"/>/common/file/download?atchFileId=<c:out value="${item.fileGrpinnb}"/>&fileSn=<c:out value="${item.fileSn}"/>" style="width:200px; height:200px;" />
					</c:if>
				</c:if>				
			</li>
</c:forEach>
...
<c:choose>
	<c:when test="${fn:length(param.file_guide)>0 }">
		${param.file_guide}
	</c:when>
	<c:otherwise>
