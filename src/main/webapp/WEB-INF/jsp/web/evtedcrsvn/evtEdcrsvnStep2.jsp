...
function fn_rsvn_sendA() {
	...
	var varEvtRsvnfmNameExists = 0;
	var varEvtRsvnfmName = "";
	$('input[name="evtRsvnfmName"]').each(function() {
		varEvtRsvnfmName = $(this).val();
		if ((varEvtRsvnfmName === undefined) || (varEvtRsvnfmName == '') || (varEvtRsvnfmName == null)) {
			varEvtRsvnfmNameExists ++;
		} else {
			$(this).val(varEvtRsvnfmName.trim());
		}
	});		
	if (varEvtRsvnfmNameExists >= 1) {
		alert("참가 가족 구성원 정보는 모든 항목을 입력해야 저장됩니다.");
		return;
	}	
	
	var varEvtRsvnfmTelnoExists = 0;
	var varEvtRsvnfmTelno = "";
	$('input[name="evtRsvnfmTelno"]').each(function() {
		varEvtRsvnfmTelno = $(this).val();
		if ((varEvtRsvnfmTelno === undefined) || (varEvtRsvnfmTelno == '') || (varEvtRsvnfmTelno == null)) {
			varEvtRsvnfmTelnoExists ++;
		} else {
			$(this).val(varEvtRsvnfmTelno.trim());
		}
	});		
	if (varEvtRsvnfmTelnoExists >= 1) {
		alert("참가 가족 구성원 정보는 모든 항목을 입력해야 저장됩니다.");
		return;
	}	
	
	if ($('#evtWantPrgmnm').val() == "") {
		if (nullObj == "") nullObj = "evtWantPrgmnm";
		$('#err_evtWantPrgmnm').text("희망프로그램을 입력해 주세요.").show();
	}
...
function fn_rsvn_sendB() {
	...
	if ($('#evtRsvnCustnm').val() == "") {
		...
		<c:if test="${evtrsvnMstVO.evtPersnGbn eq '20'}">
			...
			if ($('#evtGradeTarget').val() == "") {
				if (nullObj == "") nullObj = "evtGradeTarget";
				$('#err_evtGradeTarget').text("대상학년을 입력해 주세요.").show();
			}
...
function fn_reserve_success(data) {
	 if (data.result.resultCd == "SUCCESS") {
		 $('#evtRsvnIdx').val( data.result.resultMsg );
   	 	 $('#evtrsvnMstVO').attr("action" , "./evtEdcrsvnResult");
   		 $('#evtrsvnMstVO').submit();
   } else {
				...
				<div class="sub_cont" id="container">
					<div class="inner">
						...
						<div class="join_wrap bor">
							...
							<c:if test="${evtrsvnMstVO.evtPersnGbn eq '10'}">
							
								<form:form commandName="evtrsvnMstVO" name="evtrsvnMstVO" id="evtrsvnMstVO" action="./evtEdcrsvnAction" method="post">
										...
										<div class="table2 responsive-type mb50">
											...
											<table>
												...
												<tbody>
													...
													<tr>
														<th scope="row"><label for="custNm">이름</label></th>
														<td><c:out value="${userVO.name}"/></td>
													</tr>
													<tr>
														<th scope="row"><label for="custNm">연락처</label></th>
														<td><tags:HpDecrypt param='${userVO.ihidNum}'/></td>
													</tr>
									...
									<div class="signup2">
										<p class="stit">참가 가족 구성원 정보</p>
										<div class="table2 responsive-type mb50">
											<table>
												<caption>참가 가족 구성원 정보를 입력하는 표입니다.</caption>
												<colgroup>
													<col style="width:20%">
													<col style="width:30%">
													<col style="width:20%">
												...
												<tbody>
													...
													<tr>
														<th scope="row"><strong class="red">* </strong>이름</th>
														<th scope="row"><strong class="red">* </strong>휴대전화번호</th>
														<th scope="row"><strong class="red">* </strong>이름</th>
														<th scope="row"><strong class="red">* </strong>휴대전화번호</th>														
													</tr>
													
													<c:set var="thisEvtVeingnmprMod" value="${paramMap.evtVeingnmpr mod 2}" />
													<c:set var="thisEvtVeingnmprRms" value="${paramMap.evtVeingnmpr - 1}" />
													<fmt:parseNumber var="thisEvtVeingnmprDiv" value="${paramMap.evtVeingnmpr div 2}" integerOnly="true"/>
													
													<c:if test="${thisEvtVeingnmprMod eq '0'}">
														
														<c:forEach var="item" begin="1" end="${thisEvtVeingnmprDiv}" step="1" varStatus="status">
															
															<tr>
																<td><input type="text" name="evtRsvnfmName" placeholder="이름 입력" title="이름 입력" maxlength="50">
																	<p id="err_evtRsvnfmName" class="red" style="display:none">이름을 입력해주세요.</p>
																</td>
																<td><input type="text" id="evtRsvnfmTelno" name="evtRsvnfmTelno" placeholder="휴대전화번호입력" title="휴대전화번호입력" maxlength="50">
																	<p id="err_evtRsvnfmTelno" class="red" style="display:none">휴대전화번호를 입력해주세요.</p>
																</td>
																<td><input type="text" name="evtRsvnfmName" placeholder="이름 입력" title="이름 입력" maxlength="50">
																	<p id="err_evtRsvnfmName" class="red" style="display:none">이름을 입력해주세요.</p>
																</td>
													...
													<c:if test="${thisEvtVeingnmprMod eq '1'}">
									...
									<div class="signup2">
										<p class="stit">이용 정보</p>
										<div class="table2 responsive-type mb50">
											<table>
												<caption>이용 정보를 입력하는 표입니다.</caption>
												<colgroup>
													<col style="width:30%">
													<col style="width:70%">
												</colgroup>
												<tbody>
													...
													<tr>
														<th scope="row"><label for="evtWantPrgmnm"><strong class="red">* </strong>희망프로그램<br/>&nbsp;&nbsp;예) 시어핀스키</label></th>
														<td><input type="text" id="evtWantPrgmnm" name="evtWantPrgmnm" placeholder="희망프로그램을 입력해주세요" title="희망프로그램을 입력해주세요" maxlength="50">
															<p id="err_evtWantPrgmnm" class="red" style="display:none">희망프로그램을 입력해주세요.</p>
														</td>
													</tr>
							...
							<c:if test="${evtrsvnMstVO.evtPersnGbn eq '20'}">
								...
								<form:form commandName="evtrsvnMstVO" name="evtrsvnMstVO" id="evtrsvnMstVO" action="./evtEdcrsvnAction" method="post">
												...
												<tbody>
													...
													<c:choose>
														<c:when test="${evtrsvnMstVO.evtPersnGbn eq '20'}">
															<tr>
																<th scope="row"><label for="evtRsvnGrpnm"><strong class="red">*</strong>단체, 학교, 기관 명</label></th>
															...
															<tr>
																<th scope="row"><label for="evtVisitCustnm"><strong class="red">*</strong>방문 인솔자명</label></th>
																<td><input type="text" id="evtVisitCustnm" name="evtVisitCustnm" value="${userVO.name}"  placeholder="이름을 입력해주세요." title="방문 인솔자명을 입력해주세요" maxlength="50">
									...
									<c:if test="${detail.useinforyn eq 'Y'}">
										<div class="signup2">
											<p class="stit">이용 정보</p>
											<div class="table2 responsive-type mb50">
												<table>
													...
													<tbody>
														...
														<c:choose>
															<c:when test="${evtrsvnMstVO.evtPersnGbn eq '20'}">
																<tr>
																	<th scope="row"><label for="evtGradeTarget"><strong class="red">* </strong>대상학년 <br/>&nbsp;&nbsp;예) 중2, 고1, 초6</label></th>
																	<td><form:input path="evtGradeTarget"  placeholder="대상학년을 입력해주세요." title="대상학년을 입력해주세요." maxlength="50"/>
																		<p id="err_evtGradeTarget" class="red" style="display:none">대상학년을 입력해주세요.</p>
																	</td>
																</tr>
																<tr>
																	<th scope="row"><label for="evtWantPrgmnm"><strong class="red">* </strong>희망프로그램<br/>&nbsp;&nbsp;예) 시어핀스키</label></th>
						...
						<ul class="btn_area">
							...
							<c:if test="${evtrsvnMstVO.evtPersnGbn eq '10'}">
								<li><a href="javascript:fn_rsvn_sendA()" class="btn_ty_m_c1">예약하기</a></li>
							</c:if>
							<c:if test="${evtrsvnMstVO.evtPersnGbn eq '20'}">
								<li><a href="javascript:fn_rsvn_sendB()" class="btn_ty_m_c1">예약하기</a></li>
							</c:if>							
