			...
			<div class="sub_cont" id="container">
				<div class="inner">
						...
						<div class="cate_list">
							<div>
								...
								<ul class="cate_tab active">
									...
									<li <c:if test="${evtGbn eq 'ALL'}">class="on"</c:if>><a href="./evtEdcrsvnList" class="btn_ty_m5_c2">전체보기</a></li>
									<li <c:if test="${evtGbn eq '1001'}">class="on"</c:if>><a href="./evtEdcrsvnList?evtGbn=1001" class="btn_ty_m5_c2">강연</a></li>
									<li <c:if test="${evtGbn eq '1002'}">class="on"</c:if>><a href="./evtEdcrsvnList?evtGbn=1002" class="btn_ty_m5_c2">행사</a></li>
									<li <c:if test="${evtGbn eq '1003'}">class="on"</c:if>><a href="./evtEdcrsvnList?evtGbn=1003" class="btn_ty_m5_c2">영화</a></li>
					...
					<div class="search_wrap">
						...
						<form:form commandName="eventProgramVO" id="searchForm" name="searchForm" action="./evtEdcrsvnList?evtGbn=<c:out value='${evtGbn}'/>" method="get">
							...
							<fieldset>
								...
								<div class="search_box">
									...
									<select id="orderField" class="ml0" name="orderField" title="목록 순서를 선택하세요" onchange="fn_order_select(this.value)">
										...
										<option value='default' <c:if test="${odr eq 'default'}">selected</c:if> >단체예약 시작일</option>
										<option value='regdate' <c:if test="${odr eq 'regdate'}">selected</c:if> >최신 등록일</option>
										<option value='reqStime' <c:if test="${odr eq 'reqStime'}">selected</c:if> >단체예약신청 시작일</option>
									</select>
...
<script type="text/javascript">
	...
	function fn_drt(eventNo) {
		...
		location.href = './evtEdcrsvnDetail?evtNo='+eventNo;
	}
