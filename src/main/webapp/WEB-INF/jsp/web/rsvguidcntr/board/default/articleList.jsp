			...
			<c:if test="${!empty ctgList }">
				<ul class="tab">
					...
					<c:forEach items="${ctgList}" var="item" varStatus="status">
						<li <c:if test="${searchVO.searchCtg eq item.ctgId }">class="on"</c:if>>
							<a href="./list?searchCtg=<c:out value="${item.ctgId }"/>"><c:out value="${item.ctgNm }"/></a>
						</li>
					</c:forEach>
				</ul>
			</c:if>
				...
				<c:if test="${paginationInfo.currentPageNo < 2 }">
					...
					<c:forEach items="${noticeList}" var="resultInfo" varStatus="status">
						<tr class="n_top">
							...
							<td class="category"><c:out value='${resultInfo.ctgNm}'/></td>
