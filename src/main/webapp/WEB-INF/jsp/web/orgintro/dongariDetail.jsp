	...
	<div id="content" class="sub_content">
		...
		<div class="detail_box">
			...
			<c:choose>
				...
				<img src="<c:url value="${webDir}"/>/common/file/download?atchFileId=<c:out value="${result.atchFileId}"/>&fileSn=<c:out value="${result.atchImg}"/>" alt="학습동아리 <c:out value="${result.nttSj}"/> 이미지" onError="this.src='<c:url value="${webDir}"/>/resources/images/data/no_img02.jpg'">
				</c:when>
				<c:otherwise>
				<img src="<c:url value="${webDir}"/>/resources/images/data/no_img02.jpg" alt="이미지"/>
				</c:otherwise>
			</c:choose>
				</p>
			</div>
		...
		<div class="btn_area">
			...
			<a href="<c:url value="/web"/>/mypage/dongari/modify?nttId=<c:out value="${result.nttId}"/>" class="btn_s1_c1">수정</a>
			</c:if>
		</div>
	</div>
</body>