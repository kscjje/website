...
<form:form commandName="rsvnMasterVO" name="rsvnMasterVO" id="popFrm" action="./EXBT/exbtPop" target="rsvnPop" method="post">
	<input type="hidden" name="rsvnIdx" id="rsvId" value="${resultVO.rsvnIdx }"/>
					...
					<tr>
						...
						<td class="pro">
							<div class="pro_tt">
							 	<a href="javascript:fn_get_modTable(${resultVO.rsvnIdx}, 2)" class="w100" id="modal2<c:out value='${resultVO.rsvnIdx}'/>">
									<ul>
										...
										<li><c:choose><c:when test="${resultVO.exbtType eq '1001' }">관람</c:when><c:otherwise><c:out value="${resultVO.exbtName }"/></c:otherwise></c:choose></li>
									</ul>
								</a>
								<span>
							...
							<c:choose>
								 	<c:when test="${!empty resultVO.oid and resultVO.saleamt >0}">
										<button class="btn_ty_s3_c9" type="button" onclick="fn_move_detail('<c:out value="${resultVO.oid }" />','EXBT' , '<c:out value="${myRsvnVO.pageIndex}" />')">결제상세/취소</button>
									</c:when>
									<c:when test="${resultVO.apptype eq '20' and (resultVO.rfndSeq > 0 or resultVO.cancelAbleYn eq 'Y')}">
										<button class="btn_ty_s3_c2" type="button" onclick="fn_reserve_cancel('<c:out value="${resultVO.rsvnIdx }" />','EXBT')">예약취소</button>
									</c:when>
							</c:choose>
							</div>
						</td>
						...
						<td class="stat">
							<c:choose>
								<c:when test="${resultVO.apptype eq '12'}"><span class="stat"><c:out value="${resultVO.exbtApptypeNm}"/></span></c:when>
								<%-- <c:when test="${resultVO.apptype eq '20' and resultVO.saleamt >0}"><span class="payend"><c:out value="${resultVO.exbtApptypeNm}"/></span></c:when> --%>
								<c:when test="${resultVO.apptype eq '20'}"><span class="useend"><c:out value="${resultVO.exbtApptypeNm}"/></span></c:when>
								<c:when test="${resultVO.apptype eq '31'}"><span class="cancelend"><c:out value="${resultVO.exbtApptypeNm}"/></span></c:when>
								<c:when test="${resultVO.apptype eq '32'}"><span class="stat"><c:out value="${resultVO.exbtApptypeNm}"/></span></c:when>
							</c:choose>
						</td>
						<td class="expay red">
							<c:choose>
