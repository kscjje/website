...
%>
	...
	<form:form commandName="myRsvnVO" name="myRsvnVO" id="myRsvnVO" action="./myRsvnPay" method="post">
									...
									<c:when test="${resultVO.edcInterStatus eq 'ING'}">
										...
										<c:if test="${resultVO.edcStat eq '20'}">
											<c:if test="${resultVO.cancelYn eq 'Y'}">
												<button type="button" class="btn_ty_s2_c2" onclick="javascript:fn_waiting_cancel('EDC' , '<c:out value="${resultVO.edcRsvnReqid}"/>')">신청취소</button>
											</c:if>	
										</c:if>
		...
		<!-- //팝업 -->
		<form:form commandName="edcRsvnMstVO" name="edcRsvnMstVO" id="popFrm" action="./EDC/edcPop" target="rsvnPop" method="post">
