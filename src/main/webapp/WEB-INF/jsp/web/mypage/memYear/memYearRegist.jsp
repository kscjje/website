...
function payment_return() {
	...
	if (fDoc.document.getElementById('LGD_RESPCODE').value == "0000") {
		...
		getFormObject().action = "../../member/join/joinYearSave";
...
<c:choose>
	<c:when test="${memberYearVO.validYn eq 'Y' }">
			...
			<p class="desc mb100">미래의 과학으로 가는 지름길 국립중앙과학관의 유료회원으로 초대 합니다.</p>
	</c:when>
	<c:otherwise>
			...
			<!-- signup2 -->
<c:choose>
	<c:when test="${empty userInfo or userInfo.specialYn eq 'Y' or empty memberYearVO.itemCd }">
			<div class="signup2">
					...
					<tbody>
						<tr>
							<td class="b">
								...
								<c:if test="${userInfo.specialYn eq 'Y'}">특별회원은 유료회원을 가입하실 수 없습니다.</c:if>
	...
	<c:otherwise>
<form:form commandName="memberYearVO" name="memberYearVO" id="memberYearVO" action="./memYearSave" method="post" accept-charset="EUC-KR">
...
<c:if test="${!empty dscAmt and dscAmt > 0 }">
							<tr>
								<th scope="row" class="vtop">금액</th>
								<td><fmt:formatNumber value="${memberYearVO.saleamt}"/>원</td>
							</tr>
							<tr>
								<th scope="row" class="vtop">할인 금액</th>
								<td><fmt:formatNumber value="${dscAmt}"/>원</td>
							</tr>
</c:if>
							<tr>
								<th scope="row" class="vtop">결제 금액</th>
								<td><fmt:formatNumber value="${memberYearVO.LGD_AMOUNT}"/>원</td>
							</tr>
						</tbody>
					</table>
					<!-- //table -->
