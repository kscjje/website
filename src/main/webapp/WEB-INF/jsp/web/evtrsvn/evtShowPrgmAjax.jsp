					...
					<c:forEach items="${timeList}" var="item" varStatus="status">
						<li>
							...
							<div class="seat">
								...
								<span class="st_num"><em><fmt:formatNumber value="${(item.totCapa-item.rsvCnt)>0?(item.totCapa-item.rsvCnt):0}"/></em> <b>/</b> <fmt:formatNumber value="${item.totCapa}"/>석</span>
							</div>
							<!-- //seat -->
							<c:choose>
								...
								<c:when test="${item.rsvAbleYn eq 'Y' and item.totCapa > item.rsvCnt}">
									<c:set var="btnType" value=""/>
									<c:if test="${programVO.evtOrgMincnt < 1}"><c:set var="btnType" value=" one"/></c:if>
									<ul class="p_btn${btnType}">
										<li><a href="javascript:fn_rsv_evt('10','<c:out value="${param.evtTime }"/>','<c:out value="${item.evtTimestdSeq}"/>','<c:out value="${item.evtTimeseq}"/>')" class="btn_ty_c6">개인 예약</a></li>
										<c:if test="${programVO.evtOrgMincnt > 0 }">
											<c:choose>
												<c:when test="${programVO.evtOrgMincnt <= (item.totCapa-item.rsvCnt)}">
													<li><a href="javascript:fn_rsv_evt('20','<c:out value="${param.evtTime }"/>','<c:out value="${item.evtTimestdSeq}"/>','<c:out value="${item.evtTimeseq}"/>')" class="btn_ty_c3">단체 예약</a></li>
												</c:when>
												<c:otherwise>
