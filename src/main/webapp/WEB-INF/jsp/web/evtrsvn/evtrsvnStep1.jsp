...
<div class="sub_cont" id="container">
							...
							<!-- signup2 -->
								<div class="signup2">
									...
									<c:if test="${paramMap.evtPersnGbn eq '10'}">
										...
										<p class="stit">선택된 강연/행사/영화 예약을 확인해 주십시오.</p>
										<div class="table2 responsive-type mb50">
										
											<table>
												<caption>선택된 강연/행사/영화 예약 안내표입니다.</caption>
												<colgroup>
													<col style="width:20%">
													<col style="width:30%">
													<col style="width:20%">
												...
												<tbody>
													...
													<tr>
														<th scope="row" class="vtop">행사이름</th>
													...
													<tr>
														<th scope="row" class="vtop">관람연령가</th>
														<td class="b">
															<!-- 초등학생부터 성인까지 -->
															<c:out value="${evtData.evtTargetage}"></c:out>
														</td>
														<th scope="row" class="vtop">시간</th>
														<td class="b">
															<!-- 월수금 10:00 ~ 12:00 -->
															${fn:substring(evtTime.sTime,0,2)}:${fn:substring(evtTime.sTime,2,4)} ~ ${fn:substring(evtTime.eTime,0,2)}:${fn:substring(evtTime.eTime,2,4)}										
														</td>
													</tr>
													<tr>
											...
											<table>
												...
												<tbody>
													...
													<tr>
														...
														<th scope="row" class="vtop">분류</th>
														<td class="b">
															<c:out value="${evtData.evtGbnNm}"></c:out>
														</td>														
													</tr>
													<tr>
														<th scope="row" class="vtop">기간</th>
									...
									<!-- agree_list -->
									<p class="stit">강연/행사/영화 예약 신청 시 유의사항에 동의해 주시기 바랍니다.</p>
				...
				<form:form commandName="evtrsvnMstVO" name="evtrsvnMstVO" id="evtrsvnMstVO" action="./evtrsvnStep2" method="post">
					...
					<c:if test="${evtrsvnMstVO.dcAnnualLimit > 0}">
						<p class="desc mb5" style="text-align:left">유료회원 할인은 <span class="pink">최대 ${evtrsvnMstVO.dcAnnualLimit }명</span> 까지 가능합니다.</p>
					</c:if>
					-->
	...
	function fn_next(obj) {
		...
		*/
		...
		if (totalCnt < 1) {
			...
			alert("단체 신청 최소 인원수 미만으로 예약하실 수 없습니다.");
			...
			if (confirm("${evtData.evtOrgMincnt} 명 이상 신청하실 경우 단체로 예약하셔야 합니다.\n단체 신청페이지로 이동하시겠습니까?")) {
				window.location.replace("./evtrsvnStep1?type=20&evtTimeseq=<c:out value="${param.evtTimeseq}"/>&evtTimestdseq=<c:out value="${param.evtTimestdseq}"/>&evtNo=<c:out value="${param.evtNo}"/>&evtTime=<c:out value="${param.evtTime}"/>");
			}
		} else {
			$('#evtrsvnMstVO').submit();
		}
		...
		console.log(data);
		 ...
		 if (data.result.resultCd == "SUCCESS") {
			 $('#evtRsvnIdx').val( data.result.resultMsg );
	   	 	 $('#evtrsvnMstVO').attr("action" , "./evtrsvnIndiResult");
	   		 $('#evtrsvnMstVO').submit();
	   } else {
