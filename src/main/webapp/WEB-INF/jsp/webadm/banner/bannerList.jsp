			 	 	 ...
			 	 	 <div class="card-body">
			 	 	 	<div class="row">
							...
							<div class="col-4">
								...
								<a href="./bannerRegist" class="btn btn-info<c:if test="${commandMap.selectedMenu.insYn ne 'Y' }"> disabled</c:if>" title="등록버튼">등록</a><!-- 등록 -->
			              	</div>
	...
	<c:forEach items="${bannerGbn}" var="gbn" varStatus="status">
			<c:if test="${gbn.cd eq  resultInfo.bannerGbn}">
				<c:out value='${gbn.cdNm}'/>
								...
								<td><a href="./bannerDetail?bannerId=<c:out value="${resultInfo.bannerId}"/><c:out value="${commandMap.query}"/>"><c:out value='${resultInfo.bannerNm}'/></a></td>
								...
								<td>
								  <a class="btn bg-primary btn-xs<c:if test="${commandMap.selectedMenu.updYn ne 'Y' }"> disabled</c:if>" href="./bannerUpdt?bannerId=<c:out value="${resultInfo.bannerId}"/><c:out value="${commandMap.query}"/>">수정</a>
								</td>
		                    </tr>
	</c:forEach>
	              ...
	              <div class="card-footer p-0">
	              	<div class="pagination_2">
						<tags:AdminPaging pageInfo="${paginationInfo}" pageUrl="./bannerList?pageIndex="/>
					</div>
	            </div>
