<%
 /**
  * @Class Name : uploadRegistEVT.jsp
  * @Description : 운영관리 (강연/행사/영화) 프로그램 전용  파일업로드
	...
	function setThumbnail(event, num) {
		...
		if (flag) {
			...
			reader.onload = function(event) {
				...
				document.querySelector("div#image_container"+num).appendChild(img);
			};
...
<section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <h1>강연/행사/영화 <span style="font-size:1.2rem">> <c:out value="${programInfo.title }" /></span> </h1>
          </div>
        </div>
      </div><!-- /.container-fluid -->
</section>
			...
			<div class="card-header">
              ...
              <div class="card-body">
            	<div class="btn btn-default btn-file file_input_div">
		           ...
		           <input type="file" name="imgFileUpload" id="imgFileUpload" accept="image/*" class="file_input_hidden" onchange="setThumbnail(event,1);">
		         </div>
...
<c:forEach items="${imgFileList}" var="item" varStatus="status">
					<img src="/web/common/file/view?atchFileId=<c:out value="${item.fileGrpinnb}"/>&fileSn=<c:out value="${item.fileSn}"/>"  height="100"/>
</c:forEach>
		         </div>
           	</div>
           </div><!-- card -->
		...
		<div class="card card-primary card-outline">
			<div class="card-header">
                <h3 class="card-title">썸네일(목록) 이미지</h3>
              </div>
              <!-- /.card-header -->
              ...
              <div class="card-body">
            	<div class="btn btn-default btn-file file_input_div">
		           <i class="fas fa-paperclip"></i> 이미지 첨부
		           <input type="file" name="imgThumUpload" id="imgThumUpload" accept="image/*" class="file_input_hidden" onchange="setThumbnail(event,2);">
		         </div>
		         <ul id="imgFileList" class="pt-1">
<c:forEach items="${thumFileList}" var="item" varStatus="status">
					<li><span><c:out value="${item.orginFileName}"/> (<tags:FileSize fileSize="${item.fileSize}"/>)</span></li>
</c:forEach>
		         </ul>
	...
	<div class="card card-primary card-outline">
			<div class="card-header">
              ...
              <div class="card-body">
            	<div class="btn btn-default btn-file file_input_div">
		           <i class="fas fa-paperclip"></i> 이미지 첨부
		           <input type="file" name="imgIntroUpload" id="imgIntroUpload" accept="image/*" class="file_input_hidden" onchange="setThumbnail(event,3);">
		         </div>
		         <ul id="imgFileList" class="pt-1">
<c:forEach items="${introFileList}" var="item" varStatus="status">
					<li><span><c:out value="${item.orginFileName}"/> (<tags:FileSize fileSize="${item.fileSize}"/>)</span></li>
</c:forEach>
		         </ul>
...
<c:forEach items="${introFileList}" var="item" varStatus="status">
         ...
         <div class="col-sm-6">
      		<div class="card card-primary card-outline">
      			<div class="card-header">
                <h3 class="card-title">교육계획서(1개 이상 가능)</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
