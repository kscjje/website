...
<%
	/**
	 * @Class Name : salesSettleList.jsp
	 * @Description : 수입금정산일지 JSP
	 * @Modification Information
	 * @
	 * @  수정일              수정자            수정내용
	 * @ -------       ------    ---------------------------
	 * @ 2021.12.24     이기태           최초 생성
	 *
	 *  @author 이기태
	 *  @since 2021.12.24
	 *  @version 1.0
...
function fn_excelDown() {
	...
	$cform.attr('action', "./settleListExcel");
				...
				<div class="card">
						...
						<table class="table table-bordered table-hover text-nowrap">
							...
							<thead>
								<tr>
									...
									<th colSpan="3">합계 <i class="far fa-question-circle guide-link" style="cursor:pointer" title="합계 도움말" help-id="002"></i></th>
									<th colSpan="2">전거래일 환불 <i class="far fa-question-circle guide-link" style="cursor:pointer" title="전거래일 환불 도움말" help-id="003"></i></th>
							...
							<tbody>
								...
								<c:choose>
									...
									<c:otherwise>
										...
										<c:url value="./settleDetail" var="detailUrl">
											<c:param name="itemNm" value="${item.itemNm}" />
											<c:param name="searchOrgNo" value="${item.orgNo}" />
											<c:param name="searchStartDts" value="${searchVO.searchStartDts}" />
											<c:param name="searchEndDts" value="${searchVO.searchEndDts}" />
										</c:url>
										<tr>
											<td>${item.rnum}</td>
											<td>${item.orgNm}</td>
											...
											<td><a href="${detailUrl}"><button type="button" class="btn btn-info btn-sm">상세보기</button></a></td>
										</tr>
										</c:forEach>
									</c:otherwise>
								</c:choose>
		            ...
		            <div class="card-footer p-0">
		              	<div class="pagination_2">
							<tags:AdminPaging pageInfo="${paginationInfo}" pageUrl="./settleList?pageIndex="/>
						</div>
		            </div>
		            <!-- /.card -->
				</div>
