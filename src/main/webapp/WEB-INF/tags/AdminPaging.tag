	...
	<c:when test="${fn:indexOf(pageUrl,'javascript')==0 }">
			<ul class="pagination pagination-sm no-margin pull-right">
	    		<li class="paginate_button prev" id="dataTables-example_previous">
					...
					<c:if test="${pageInfo.firstPageNoOnPageList >  pageGroupSize}"><a href="${pageUrl}(${pageInfo.firstPageNoOnPageList - pageGroupSize})">Previous</a></c:if>
				</li>
				<c:forEach var="pageIndex" begin="${pageInfo.firstPageNoOnPageList}" end="${pageInfo.lastPageNoOnPageList}"><li class="paginate_button<c:if test="${pageIndex == pageInfo.currentPageNo}"> current</c:if>"><a href="${pageUrl}(${pageIndex})">${pageIndex}</a></c:forEach>
				<li class="paginate_button next" id="dataTables-example_next">
				<c:if test="${pageInfo.lastPageNoOnPageList >= pageInfo.totalPageCount}"><a >Next</a></c:if><c:if test="${pageInfo.lastPageNoOnPageList <  pageInfo.totalPageCount}"><a href="${pageUrl}(${pageInfo.firstPageNoOnPageList + pageGroupSize})">Next</a></c:if>
				</li>
			</ul>
	</c:when>
	<c:otherwise>
			<ul class="pagination pagination-sm no-margin pull-right">
				...
				<c:forEach var="pageIndex" begin="${pageInfo.firstPageNoOnPageList}" end="${pageInfo.lastPageNoOnPageList}"><li class="paginate_button<c:if test="${pageIndex == pageInfo.currentPageNo}"> current</c:if>"><a href="${pageUrl}${pageIndex}<%=param%>">${pageIndex}</a></c:forEach>
				<li class="paginate_button next" id="dataTables-example_next">
