...
<form:hidden path="pageIndex" />
 	   ...
 	   <div class="row mb-2">
			 ...
			 <div class="col-8" style="text-align:right">
			 	<button type="button" onclick="fn_Rest()" class="btn  btn-secondary btn-sm btn-flat">Rest테스트</button>
              ...
              <div class="card-body table-responsive p-0">
	               <table class="table table-bordered table-hover text-nowrap">
	                  ...
	                  <thead>
	                    <tr>
	                      	<th rowspan="2">번호</th>
	                      	<th rowspan="2">기관 <span id="orgNm" class="sort">▼</span></th>
	                      	<th rowspan="2">프로그램명 <span id="prgNm" class="sort">▼</span></th>
	                      	<th rowspan="2">접수기간</th>
							<th colspan="3">정원</th>
							<th rowspan="2">교육기간</th>
							<th rowspan="2">접수방식 <span id="rectType" class="sort">▼</span></th>
							<th rowspan="2">수업요일</th>
...
<c:forEach items="${programList}" var="item" varStatus="status">
		                    <tr>
								...
								<td>
									<a href="./edcProgramUpdt?edcPrgmid=<c:out value="${item.edcPrgmNo}"/><c:out value="${commandMap.query}"/>"><c:out value='${item.edcPrgmnm}'/></a>
								</td>
								...
								<td><c:choose><c:when test="${item.edcCapaDvdyn eq 'Y'}"><fmt:formatNumber value='${item.edcOncapa}'/></c:when><c:otherwise>-</c:otherwise></c:choose></td>
								<td><c:choose><c:when test="${item.edcCapaDvdyn eq 'Y'}"><fmt:formatNumber value='${item.edcOffcapa}'/></c:when><c:otherwise>-</c:otherwise></c:choose></td>
								<td><hisco:DateUtil datestr="${item.edcSdate}" format="yy.MM.dd"/> ~ <hisco:DateUtil datestr="${item.edcEdate}" format="yy.MM.dd"/></td>
								<td><c:out value='${item.rsvnTypeNm}'/></td>
								...
								<td>
								  	<a class="btn bg-primary btn-xs<c:if test="${commandMap.selectedMenu.insYn ne 'Y' }"> disabled</c:if>" href="./edcProgramRegist?copyPrgmid=<c:out value="${item.edcPrgmid}"/><c:out value="${commandMap.query}"/>">복사</a>
								</td>
		                    </tr>
</c:forEach>
	                  </tbody>
              ...
              <div class="card-footer p-0">
              	<div class="pagination_2">
					<tags:AdminPaging pageInfo="${commandMap.pagingInfo}" pageUrl="./edcProgramList"/>
				</div>
            </div>
