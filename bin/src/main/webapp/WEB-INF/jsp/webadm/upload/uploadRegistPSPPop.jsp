...
<section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <h1>사업장 <span style="font-size:1.2rem">> <c:out value="${programInfo.title }" /></span> </h1>
          </div>
        </div>
      </div><!-- /.container-fluid -->
</section>
		...
		<div class="row">
			<div class="col-sm-6">
      		<div class="card card-primary card-outline">
                ...
                <h3 class="card-title">소개 이미지 (1개 이상 가능)</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
...
<input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }" />
				<div class="btn btn-default btn-file file_input_div">
		           ...
		           <input type="file" name="file_1"  id="egovComFileUploader" accept="image/*"  class="file_input_hidden"  onchange="setThumbnail(event);">
		         </div>
		         <ul id="egovComImgList" class="pt-1">
		...
		<c:forEach items="${imgFileList}" var="item" varStatus="status">
					...
					<a href="javascript:;" onclick="fn_file_delete(this,'${item.fileSn}')" style="margin-left:5px" class="badge bg-danger">X</a></span>
						<br>
						<img src="/web/common/file/view?atchFileId=<c:out value="${item.fileGrpinnb}"/>&fileSn=<c:out value="${item.fileSn}"/>"  width="50%"/>
					</li>
		</c:forEach>
				</ul>
...
<div class="col-sm-6">
	<div class="card card-primary card-outline">
			<div class="card-header">
                <h3 class="card-title">썸네일 이미지 (1개)</h3>
              </div>
              <!-- /.card-header -->
              ...
              <div class="card-body">
            	<div class="btn btn-default btn-file file_input_div">
		           <i class="fas fa-paperclip"></i> 이미지 첨부
		           <input type="file" name="imgThumUpload" id="imgThumUpload" accept="image/*" class="file_input_hidden" onchange="setThumbnail(event);">
		         </div>
...
<c:forEach items="${thumFileList}" var="item" varStatus="status">
					<img src="/web/common/file/view?atchFileId=<c:out value="${item.fileGrpinnb}"/>&fileSn=<c:out value="${item.fileSn}"/>"  width="60%"/>
</c:forEach>
		         </div>
           	</div>
           </div><!-- card -->
