				...
				$.ajax({
				      ...
				      success: function(data) {
				    	   if (data.result.code == "ERROR") {
					        	 ...
					        	 alert(data.result.msg);
						         window.location.replace("./bannerList<c:out value="${searchQuery}"  escapeXml="false" />");
					         }
				      }
				 });
			}
	...
	<c:forEach items="${fileList}" var="item" varStatus="status"><c:if test="${status.index == 0 }">
	 ...
	 <c:choose>
		<c:when test="${commandMap.selectedMenu.updYn eq 'Y' }">
			<a class="btn btn-info mr-2 float-right" href="./bannerUpdt<c:out value="${commandMap.queryAll}"/>">수정</a>
		</c:when>
		<c:otherwise>
			<button type="button" class="btn btn-info mr-2 float-right disabled">수정</button>
		</c:otherwise>
	</c:choose>
	                  	<a class="btn btn-default " href="./bannerList<c:out value="${searchQuery}"  escapeXml="false" />">목록</a>
	                </div>
	                <!-- /.card-footer -->
