...
<%
 /**
  * @Class Name : groupAdmList.jsp
  * @Description : 관리자 권한 목록 JSP
...
<c:forEach items="${list}" var="item" varStatus="status">
	                    <tr>
							...
							<td>
								<c:choose>
								 	<c:when test="${item.parentRole eq prevAuthorCode }">&nbsp;<i class="fa fa-fw  fa-caret-right"></i></c:when>
								</c:choose>
								<c:out value='${item.authorCode}'/>
								<c:set var="prevAuthorCode" value="${item.authorCode}" />
							</td>
							...
							<td>
									...
									<a href="<c:url value="./checkGroupMenuList"/>?authorCode=${item.authorCode}" class="btn bg-primary btn-xs" ><i class="fa fa-fw fa-share"></i> 메뉴 권한 관리</a>
	                         </td>
						</tr>
</c:forEach>
	                  </tbody>
