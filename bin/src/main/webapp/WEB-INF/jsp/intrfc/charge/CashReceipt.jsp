...
<%
	...
	//System.out.println("call CashReceipt.jsp");
    ...
    String LGD_TID                = request.getParameter("LGD_TID");			  		  //토스페이먼츠 거래번호 (취소시만 사용) 
	...
	*/
    ...
    //String configPath 			  = "D:/workspace/XPay/lgdacom";  										  //토스페이먼츠에서 제공한 환경파일("/conf/lgdacom.conf") 위치 지정.
    ...
    if (LGD_METHOD.equals("AUTH")) {    // 현금영수증 발급 요청 
		...
		if (LGD_AMOUNT !=null && LGD_AMOUNT > 0) {
				xpay.Set("LGD_AMOUNT", LGD_AMOUNT);	//부분취소금액
		}
		
    	if (LGD_PAYTYPE.equals("SC0040")) {  //가상계좌건 현금영수증 발급취소시 필수  
			xpay.Set("LGD_SEQNO", "001");
			//xpay.Set("LGD_AMOUNT", LGD_AMOUNT); //부분취소 금액(전체취소 시 불필요)
    	}
    }
