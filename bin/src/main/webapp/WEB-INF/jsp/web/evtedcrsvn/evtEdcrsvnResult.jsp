...
<div class="sub_cont" id="container">
	<div class="inner">
		...
		<div class="join_wrap bor">
			...
			<div class="signup2">
			<c:choose>
				<c:when test="${resultVO.evtPersnGbn eq '10' }">
					<p class="desc desc3 mb35">가족 예약이 완료 되었습니다.</p>
				</c:when>
				<c:when test="${resultVO.evtPersnGbn eq '20' }">
					<p class="desc desc3 mb35">단체교육 예약이 완료 되었습니다.</p>
				</c:when>				
			<c:otherwise>
					<c:choose>
						<c:when test="${resultVO.evtPersnGbn eq '2001'}">
							<p class="desc desc3 mb5">단체교육 예약이 완료 되었습니다.</p>
				...
				<div class="table2 responsive-type mb50">
					...
					<table>
						...
						<tbody>
							...
							<c:if test="${resultVO.evtPersnGbn eq '10'}">
								...
								<tr>
									<th scope="row" class="vtop">참가 가족 구성원 정보</th>
									<td class="b">
										...
										<table>
											...
											<tbody>
												...
												<c:forEach items="${grpRsvnFamlyinfo}" var="item" varStatus="status">
													<tr>													
														<td class="b">
															${item.evtRsvnfmName}
														</td>
														<td class="b">
															<tags:HpDecrypt param="${item.evtRsvnfmTelno}"/>
														</td>
													</tr>													
												</c:forEach>											
								...
								<tr>
									<th scope="row" class="vtop">희망프로그램</th>
									<td class="b">
										<c:out value="${resultVO.evtWantPrgmnm}"/>
									</td>
								</tr>								
							
							</c:if>

							<c:if test="${resultVO.evtPersnGbn eq '20'}">
								<tr>
									<th scope="row" class="vtop">교육명</th>
									<td class="b">
										<c:out value="${resultVO.evtName}"/>
								...
								<c:if test="${resultVO.useinforYn eq 'Y'}">
									<tr>
										<th scope="row" class="vtop">대상학년</th>
										<td class="b">
											<c:out value="${resultVO.evtGradeTarget}"/>
										</td>
									</tr>
									<tr>
										<th scope="row" class="vtop">희망프로그램</th>
			...
			<c:if test="${resultVO.evtPersnGbn eq '10'}">
				<div class="btn_area">
					...
					<li><a href="/web/mypage/myRsvn/myRsvnList?gubun=EVTEDC" class="btn_ty_m3_c1">예약현황보기</a></li>
					<li><a href="./evtEdcrsvnList" class="btn_ty_m3_c5">메인화면으로</a></li>					
				</div>
			</c:if>
			<c:if test="${resultVO.evtPersnGbn eq '20'}">
				<div class="btn_area">
		...
		<ul class="btn_area long" style="display:none;">  
			<li><a href="./evtrsvnList" class="btn_ty_m_c1">단체교육 목록</a></li>
			<c:choose>
				<c:when test="${amount > 1 }">
