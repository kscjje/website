				...
				<c:forEach items="${edcarsvnMainList}" var="item" varStatus="status">
						...
						<li>
							...
							<c:choose>
								<c:when test="${item.eduInterStatus eq 'ING'}">
									<div class="seat">
										<span class="st_num"><em><fmt:formatNumber value="${(item.edcPncpa - item.appCnt) > 0 ? (item.edcPncpa - item.appCnt) : 0 }" pattern="#,###" /></em> <i>팀 신청가능</i> <b>/</b> 총 <fmt:formatNumber value="${item.edcPncpa}" pattern="#,###" />팀</span>
									</div>
									<ul class="p_btn">
										<li>
