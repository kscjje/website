...
<form:hidden path="gubun" />
						...
						<table>
							<caption>관람 예약 신청현황 게시판</caption>
							<thead>
								<tr>
									<th class="number">번호</th>
									<th class="numdt">예약번호/예약일자</th>
									<th class="">관람분류/관람일자</th>
								...
								<c:forEach items="${list}" var="resultVO" varStatus="status">
									...
									<tr>
										...
										<td class="numdt left">
											...
											<%-- <a href="#" data-toggle="modal" data-target="#modal1"><c:out value="${resultVO.rsvnNo}"/></a> --%>
											<a href='#' onclick="fn_get_modTable('${resultVO.rsvnIdx}'); return false;"><c:out value="${resultVO.rsvnNo}"/></a>
											
											<!-- 
											<a href="javascript:fn_get_modTable(${resultVO.rsvnIdx})"><c:out value="${resultVO.rsvnNo}"/></a>
											-->
										...
										<td class="pro">
											<div class="pro_tt">
												...
												<a href="#" data-toggle="modal" data-target="#modal1">
													<ul>
														<li><c:out value="${resultVO.exbtName}"/></li>
													</ul>
												</a>
												-->
												
												<a href='#' onclick="fn_get_modTable('${resultVO.rsvnIdx}'); return false;"><c:out value="${resultVO.exbtName}"/></a>
										...
										<td class="stat cancelend">
											...
											<c:choose>
												<c:when test="${resultVO.interStatus eq 'ING'}">
												
													<c:if test="${resultVO.apptype eq '20'}">
														<button type="button" class="btn_ty_s2_c2" onclick="javascript:fn_waiting_cancel('EXBT' , '<c:out value="${resultVO.rsvnIdx}"/>')">신청취소</button>	
													</c:if>
										...
										<td class="expay red">
											<fmt:parseDate var="cancelDate" value="${resultVO.cancelDate}" pattern="yyyy-MM-dd"/>
			...
			<table>
				...
				<c:forEach items="${list}" var="resultVO" varStatus="status">
					<tr>
						<td class="chek">
							<input type="checkbox" name="rsvnIdx" value="${resultVO.rsvnIdx}" data-amt="${resultVO.payamt}" id="rsvnIdx${status.index}"/>
							<label for="rsvnIdx${status.index}">선택</label>
						</td>
						<td class="numdt left">
							<a href="javascript:fn_get_modTable(${resultVO.rsvnIdx})"><c:out value="${resultVO.rsvnNo}"/></a>
							<br><fmt:formatDate value="${resultVO.regdate}" pattern="yyyy.MM.dd HH:mm"/>
						</td>
						<td class="pro">
							<div class="pro_tt">
								<ul>
									<li><c:out value="${resultVO.partNm}"/></li>
									<li><c:out value="${resultVO.exbtTypeNm}"/></li>
								</ul>
								<span>
								<fmt:parseDate var="extYmd" value="${resultVO.ymd}" pattern="yyyyMMdd"/>
								<fmt:formatDate value="${extYmd}" pattern="yyyy.MM.dd"/>
							...
							<div class="btn_pro">
									<c:choose>
										<c:when test="${resultVO.editYn eq 'Y'}">
												<button type="button" class="btn_ty_s3_cty" onclick="location.href='./myRsvnExbtModify?rsvnIdx=${resultVO.rsvnIdx}'">예약수정</button>
										</c:when>
										<c:otherwise>
						...
						<td class="pay"><fmt:formatNumber value="${resultVO.payamt}"/>원</td>
						<td class="stat"><c:out value="${resultVO.exbtApptypeNm}"/></td>
						<td class="expay red">
							<c:choose>
								<c:when test="${empty  resultVO.paywaitEnddt}">관람 시작전까지</c:when>
								<c:otherwise>
									<fmt:formatDate value="${resultVO.paywaitEnddt}" pattern="yyyy.MM.dd"/>
									<em><fmt:formatDate value="${resultVO.paywaitEnddt}" pattern="HH:mm"/></em>
								</c:otherwise>
							</c:choose>
						</td>

						<td class="extra"><button type="button" class="btn_ty_s2_c6"  onclick="javascript:fn_waiting_cancel('EXBT' , '<c:out value="${resultVO.rsvnIdx}"/>')">예약취소</button></td>
					</tr>
			</c:forEach>
				</tbody>
			</table>
...
<form:form commandName="rsvnMasterVO" name="rsvnMasterVO" id="popFrm" action="./EXBT/exbtPop" target="rsvnPop" method="post">
...
<script>
	...
	function fn_waiting_cancel(gubun, rsvnIdx) {
		...
		if (confirm("해당 관람 예약을 정말 취소하시겠습니까?")) {
