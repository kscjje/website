...
<script>
    ...
    function fn_sel_birthdate(val  ,eleName){
    	console.log("val : " + val);
    	console.log("eleName : " + eleName);
    	
    	$("#"+eleName+"Text").text(val);
    	$("input[name="+eleName+"]").val(val);
    	//document.getElementsByName("eleName").value=val;
    }
</script>
     ...
     <div class="sub">
            ...
            <div class="myTable-wrap pt-0">
              ...
              <table>
                ...
                <tbody>
                        ...
                        <div class="input-group">
                          <label for="inp09_04_01" class=" visually-hidden"></label>
							...
							<c:if test="${type eq 'child'}">
	                          <input type="text" id="memNm" name="memNm" class="inp02_box w280" value="">
							</c:if>
                        ...
                        <div class="d-flex">
                          <div class="searchBar-item01 me-3">
                            <div class="dates">
                              ...
                              <input type="text" autocomplete="off" id="datapiker_08_01_01" class="form-control datepicker birth" name="birthDatePicker" value="">
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <tr>
                    <th scope="row">성별<i class="star">*</i></th>
                    <td colspan="3">
                      <div class="radio">
                        <span>
                        	<input type="radio" id="gender01" name="gender"  value="M" />
                          <label for="gender01">남자</label>
                        </span>
                        <span>
                        	<input type="radio" id="gender02" name="gender" value="F"/>
                          <label for="gender02">여자</label>
                        </span>
                      </div>
                      <p class="myTable-note">
                        <span class="star-green">*</span> 성별을 선택해주세요
                      </p>
                    </td>
                  </tr>
		...
		var validMember = {
			...
			genderCheck:function() {
				...
				if (   (varGender === undefined) || (varGender == null) || (varGender == '')   ) {
					alert("성별을 선택해주세요");
			...
			birthCheck:function() {
				var birthDatePicker = $('input[name=birthDatePicker]').val();
				
				if(   (birthDatePicker === undefined) || (birthDatePicker == null) || (birthDatePicker == '')   ){
					alert("생년월일을 선택해주세요");
					$('input[name=birthDate]').focus();
					return false;
				}
				//alert(birthDatePicker.length);
				var birthDay = birthDatePicker.substring(birthDatePicker.length-2, birthDatePicker.length   );//substring("시작 위치", "종료 위치") 
				var birthMonth = birthDatePicker.substring(birthDatePicker.length-4,birthDatePicker.length-2);//substring("시작 위치", "종료 위치") 
				var showindex =  birthDatePicker.length - (birthDay+birthMonth).length;
				//alert("showindex : "+showindex);
				var birthYear = birthDatePicker.substr( 0 , showindex);//substr("시작 위치", "길이")
				...
				var birthDate = new Date(Number(birthYear), Number(birthMonth)-1,  Number(birthDay));//Date 객체에서 월은 0~11 값,
				...
				$('input[name=birthDate]').val(birthYear +"-" + birthMonth + "-" + birthDay);
			...
			chkAll : function() {
				var chkList = [validMember.resdncGbn(), validMember.passDiff(), validMember.passCheck(), validMember.email(), validMember.userid(), validMember.address(), validMember.smsYn(), validMember.hpCheck() , validMember.genderCheck(), validMember.birthCheck() ];
				var flag = true;
				var idFlag = false;
				var emailFlag = false;
